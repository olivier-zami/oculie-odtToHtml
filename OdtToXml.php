...
class OdtToXml
{
    ...
    */
    ...
    public static function parse($odt_file, $xml_string=NULL)
    {
         ...
         */
            //global self::$_ophir_odt_import_conf;
            ...
            if ($xml_string===NULL){
                ...
                if (isset($content) && @$xml->xml($content) === FALSE) {
                    self::ophir_error("Unable to read file contents.");
                    return false;
                }
            }else{
                if(@$xml->xml($xml_string)===FALSE) {
                    self::ophir_error("Invalid file contents.");
                    return false;
                }
            }
                ...
                elseif (in_array($xml->nodeType,
                    ...
                    switch ($xml->name) {
                        case "#text"://Text
                            ...
                            break;
                        case "text:h"://Title
                            if (self::$_ophir_odt_import_conf["features"]["header"]===0) {
                                $xml->next();
                                break;
                            }
                            elseif (self::$_ophir_odt_import_conf["features"]["header"]===1) break;
                        ...
                        case "text:a":
                            if (self::$_ophir_odt_import_conf["features"]["link"]===0) {
                                $xml->next();
                                break;
                            }
                            elseif (self::$_ophir_odt_import_conf["features"]["link"]===1) break;
